...
def add_avg_volume(df, df_new):
    df_new['avg_volume_5'] = df['Close'].rolling(5).mean().shift(1)
    df_new['avg_volume_30'] = df['Close'].rolling(21).mean().shift(1)
    df_new['avg_volume_365'] = df['Close'].rolling(252).mean().shift(1)
...
def generate_features(df):
    ...
    @return: dataframe, dataset with new features
    """
    df_new = pd.DataFrame()
    # 6 original features
